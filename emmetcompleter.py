        ...
        for tag in tags:
            ...
            if 'attributes' in tag_value:
                ...
                if not joined_attribute in _reuse_map:
                    ...
                    _reuse_map[joined_attribute] = sortlist.SortList(list(attributes.values()),_lambda_name_getter)
    ...
    class Completer():
        ...
        def getCompletions(self,line,column,project,type,name,buffer):
                ...
                for i in range(column-1,-1,-1):
                    if active_line[i] == '>' or active_line[i] == '<':
                        in_line = True
                        break
                if in_line == False:
                    rolling = ''
                    for i in range(line-1,-1,-1):
                        bline=buffer[i]
                        for j in range(len(bline)-1,-1,-1):
                            ch = bline[j]
                            if ch != '\n' and ch != '>':
                                rolling = ch + rolling
                            else:
                            ...
                            if ch == '>' or ch == '<':
                                i=-1
                                break
                    active_line = rolling + active_line
